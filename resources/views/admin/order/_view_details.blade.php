      ...
      <div class="ms-panel">
        <div class="ms-panel-body">
          <div class="row" id="section-to-print">
            <div class="col-12">
              <h3 class="text-center d-none" id="section-title">Order Details</h3>
            </div>
            <div class="col-6 p-0">
              <div class="col-12 mt-5" id="section-to-print-logo" style="font-size: 12px; line-height: 0.5;">
                <img src="{{ asset('assets\images\logo_long.jpg') }}" style="height: 100px; display: none; margin-bottom: 15px" alt="">
                <div>
                  <p style="line-height: 15px;">1/A-B, College Street, Mirpur Road, (Near Science Laboratory Police Box), Dhaka-1205, Bangladesh</p>
                  <p style="line-height: 15px;">alhadienterprise@gmail.com</p>
                  <p style="line-height: 15px;">Phone: 00000000</p>
                  <p style="line-height: 15px;">Mobile : 01900000000</p>
                </div>
              </div>
            </div>
            <div class="col-6 mt-5 p-0" id="section-customer">
              <div class="text-right mr-3" style="font-size: 12px; line-height: 0.5;">
                <p style="line-height: 15px;">{{ $user->name }}</p>
                @if(isset($user->email))
                  <p style="line-height: 15px;">{{ $user->email }}</p>
                @endif
                <p style="line-height: 15px;">{{ $user->mobile_no }}</p>
                <p style="line-height: 15px;">{{ $order->delivery_address }}</p>
                <p class="text-capitalize" style="line-height: 15px;">Order Status: {{ $order->status }}</p>
              </div>
            </div>
            <div class="col-12">
                    ...
                    <table class="table table-hover mb-0 border">
                      ...
                      @foreach($product_details as $row)
                        ...
                        <tr >
                          ...
                          <td class="t-border-r">{{ $row->quantity }} @if($row->attributes->color != "no color") <span class="btn" style="width: 10px; height: 10px; background: {{ $row->attributes->color }}"></span> @endif</td>
                      ...
                      <tr class="text-right">
                        ...
                        <td colspan="2" class="p-0">
                          <table class="w-100 border-left border-top-0" style="min-width: 100%;">
                            ...
                            <tr>
                              <td class="text-right text-uppercase"><strong>VAT(5%)</strong></td>
            ...
            <div class="col-12">
              ...
              <!-- Card -->
              <h3 class="card-title mt-4 mt-2">Payment Details</h3>
              <div class="dt-card overflow-hidden">
                <!-- Card Body -->
                  ...
                  <div class="table-responsive">
                    <table class="table table-hover mb-0">
                      ...
                      @foreach($payment_details as $row)
                        <tr class="t-border-b">
                          <th scope="row" class="t-border-r t-border-l">{{ $i++ }}</th>
                          <td class="t-border-r">{{ date('F d, Y h:i A', strtotime($row->created_at)) }} </td>
          ...
          <div class="row mb-3">
            <div class="col-6 mt-3 mb-4 d-flex text-center">
              ...
              <form action="{{ route('admin.order.change.status') }}" method="POST" class="d-flex">
                ...
                <select name="status" class="form-control p-0 m-0" data-id="{{ $order->id }}" style="width: 120px">
                  <option value="" selected>Select one</option>
                  <option value="Pending" @if(strtolower($order->status) == 'pending') selected @endif>Pending</option>
                  <option value="Processing" @if(strtolower($order->status) == 'processing') selected @endif>Processing</option>
                  <option value="Delivered" @if(strtolower($order->status) == 'delivered') selected @endif>Delivered</option>
                  <option value="Completed" @if(strtolower($order->status) == 'completed') selected @endif>Completed</option>
                  <option value="Canceled" @if(strtolower($order->status) == 'canceled') selected @endif>Canceled</option>
                </select>
              </form>
            </div>
            <div class="col-6 text-right">
              <button type="button" class="btn btn-success btn-sm mt-3 mb-4" onclick="window.print()"> <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
...
</svg> <span class="text-white">Print</span></button>
            </div>
          </div>
        </div>
      </div>
